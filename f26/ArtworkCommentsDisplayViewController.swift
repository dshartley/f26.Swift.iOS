//
//  ArtworkCommentsDisplayViewController.swift
//  f26
//
...
/// A ViewController for the ArtworkCommentsDisplay
public class ArtworkCommentsDisplayViewController: UIViewController {
	
	// MARK: - Private Stored Properties
	
	fileprivate var controlManager:								ArtworkCommentsDisplayControlManager?
	fileprivate var artworkID: 									Int = 0
	...
	public var delegate:										ProtocolArtworkCommentsDisplayViewControllerDelegate?
	...
	fileprivate func setupControlManager() {
		...
		self.controlManager = ArtworkCommentsDisplayControlManager()
		
		self.controlManager!.artworkID = self.artworkID
	...
	fileprivate func setupModelManager() {
		...
		ModelFactory.setupArtworkCommentModelAdministrator(modelManager: self.controlManager!.modelManager! as! ModelManager)
	}
	
	fileprivate func setupViewManager() {
		
		// Create view strategy
		let viewAccessStrategy: ArtworkCommentsDisplayViewAccessStrategy = ArtworkCommentsDisplayViewAccessStrategy()
		
		viewAccessStrategy.setup(postedByNameTextField:		self.postedByNameTextField,
		                         commentTextView:			self.commentTextView)
		
		// Setup the view manager
		self.controlManager!.viewManager = ArtworkCommentsDisplayViewManager(viewAccessStrategy: viewAccessStrategy)
	}
	...
	fileprivate func createCommentsTableViewCell(for dataItem: ArtworkCommentWrapper, at indexPath: IndexPath) -> CommentsTableViewCell {
	...
	fileprivate func loadArtworkComments() {
		
		// Create completion handler
		let loadArtworkCommentsCompletionHandler: (([ArtworkCommentWrapper]?, Error?) -> Void) =
		{
			(items, error) -> Void in
		...
		self.controlManager!.loadArtworkComments(oncomplete: loadArtworkCommentsCompletionHandler)
	}
	...
	@IBAction func closeViewButtonTapped(_ sender: Any) {
		...
		// Notify the delegate
		self.delegate?.artworkCommentsDisplayViewController(didDismiss: self)
	...
	@IBAction func postCommentButtonTapped(_ sender: Any) {
		...
		// Create completion handler
		let postCommentCompletionHandler: ((ArtworkCommentWrapper?, Error?) -> Void) =
		{
			(item, error) -> Void in
			...
			if (item != nil && error == nil) {
				...
				DispatchQueue.main.async(execute: {
					...
					// Notify the delegate
					self.delegate?.artworkCommentsDisplayViewController(didPostComment: self)
				})
			}
...
extension ArtworkCommentsDisplayViewController : UITextFieldDelegate {
...
extension ArtworkCommentsDisplayViewController : UITextViewDelegate {
...
extension ArtworkCommentsDisplayViewController : UITableViewDelegate, UITableViewDataSource {
	
	public func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {

		return self.controlManager!.artworkCommentWrappers.count
	...
	public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		// Get the item
		let artworkCommentWrapper: ArtworkCommentWrapper	= self.controlManager!.artworkCommentWrappers[indexPath.row]

		// Create the cell
		let cell = self.createCommentsTableViewCell(for: artworkCommentWrapper, at: indexPath)
